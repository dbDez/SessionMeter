...
/// PER-MODEL weekly walls (e.g. a <c>weekly_scoped</c> entry scoped to model "Fable") that the top-level
/// fields never expose. Every entry is parsed dynamically so the binding calculation cannot sail past a
/// per-model wall that is exhausted while the all-models windows still read low.
/// </summary>
    ...
    bool IsActive)
{
    ...
    /// <summary>
    /// True when this window is scoped to a SINGLE model (a per-model weekly wall, e.g. Fable) — its
